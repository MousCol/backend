        ...
        for field_name, field_type in available_fields.items():
            ...
            if isinstance(attribute, list):
                ...
                for element in attribute:
                    transformed_data[field_name].append(
                        FromDictMixin.attribute_to_dict(element)
                    )
            elif isinstance(attribute, dict):
                transformed_data[field_name] = {}
                for key, element in attribute.items():
                    transformed_data[field_name][key] = FromDictMixin.attribute_to_dict(element)
            elif isinstance(attribute, enum.Enum):
                transformed_data[field_name] = attribute.value
            elif isinstance(attribute, datetime.datetime):
                transformed_data[field_name] = iso_from_datetime(attribute)
            else:
                transformed_data[field_name] = FromDictMixin.attribute_to_dict(attribute)
        return transformed_data


    @classmethod
    def from_dict(cls, data):
        built_in_types = (int, str, bool, float, datetime.time, datetime.datetime)
        collection_types = (list, dict)
        available_fields = {field.name: field for field in dataclasses.fields(cls)}
        transformed_data = {}
        for key, value in data.items():
            if key in available_fields.keys() and value:
                matching_internal_field = available_fields[key]
                if matching_internal_field.type in built_in_types:
                    internal_value = value
                elif matching_internal_field.type == datetime.datetime and value:
                    #TODO Sometimes not take the correct time and return null see birth dates
                    ...
                    internal_value = None
                if internal_value:
                    transformed_data[key] = internal_value
